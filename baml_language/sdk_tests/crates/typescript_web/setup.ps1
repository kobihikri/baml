...
Get-ChildItem -Directory | ForEach-Object {
    ...
    if (Test-Path $generated) {
        ...
        if ($null -eq $PlaywrightFixture) { $PlaywrightFixture = $generated }
    }
}

# Match the existing webview browser-test workflow: install the
# Playwright-managed Chromium build instead of requiring system Chrome.
if ($null -ne $PlaywrightFixture) {
    Push-Location $PlaywrightFixture
    try {
