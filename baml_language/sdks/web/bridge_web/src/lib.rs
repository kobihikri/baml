...
pub fn stage_runtime_bytecode(bytecode: &[u8], callbacks: &JsValue) -> Result<(), JsError> {
    let callbacks = callbacks.unchecked_ref::<sys_wasm::BrowserCallbacks>();
    let sys_ops = sys_wasm::build(callbacks);
